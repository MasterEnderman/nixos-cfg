...
modules/
  nixos/
    features/
      base.nix                              ← Shared defaults (user account, FS flags)
      common.nix                            ← Shared system config
      kernel.nix                            ← CachyOS kernel + Lantian cache
      noctalia.nix                          ← Noctalia Cachix cache
      niri.nix                              ← Compositor + Autologin
      preservation.nix                      ← tmpfs root + persistent /persist
    hosts/
      hp-probook/                           ← One folder per machine
        default.nix                         ← Host declaration (imports features)
        hardware-configuration.nix          ← Generated by nixos-generate-config
        disko.nix                           ← Disk layout
  home/
    enderman/                               ← Single user config (shared across machines)
      default.nix                           ← Auto-imports sibling .nix files
      git.nix
      noctalia.nix
      terminal.nix
      zen.nix                               ← Browser with custom search engines
```

## Features

```
✓ Zero relative imports — auto-discovery for user modules
✓ DRY host configuration — mkHost helper for easy scaling (8+ machines)
✓ Impermanent root (/) with /persist for persistence
✓ LUKS encryption + Btrfs subvolumes
✓ Niri Wayland compositor + Tuigreet autologin
✓ Noctalia shell spawns automatically after login
✓ Zen browser with DuckDuckGo default + Nix shortcuts (@np, @no, @nw, etc.)
✓ CachyOS performance kernel
✓ Preserved SSH keys, configs, downloads, projects
```
...
```
[ ] modules/nixos/hosts/hp-probook/disko.nix — device path matches your actual disk (/dev/nvme0n1 or adjust)
[ ] modules/nixos/hosts/hp-probook/hardware-configuration.nix — regenerated in step 5 and looks sane
[ ] modules/nixos/features/common.nix — timezone and locale are correct
[ ] modules/nixos/features/niri.nix — keyboard layout is correct ("de" by default)
[ ] modules/home/enderman/git.nix — name and email are correct
[ ] modules/nixos/features/kernel.nix — kernel variant is what you want (linuxPackages-cachyos-bore by default)
[ ] modules/nixos/features/base.nix — initialPassword is set to something temporary!
```
...
4. greetd + tuigreet logs in AS ENDERMAN AUTOMATICALLY (no second password!)
5. Niri starts, noctalia-shell launches on top
```
